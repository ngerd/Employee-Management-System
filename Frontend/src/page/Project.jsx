...
const Project = () => {
    ...
    const projects = [
        { name: "Web development", duration: "55/01/2025-03/03/2025", manager: "John", code: "1234" },
        { name: "Capstone", duration: "1/01/2025-03/03/2025", manager: "YN", code: "1573" },
        { name: "Game development", duration: "21/01/2025-03/04/2025", manager: "Jun", code: "G456" },
    ];

    const [filters, setFilters] = useState({ name: [], duration: [], manager: [], code: [] });
    const [showDropdown, setShowDropdown] = useState({ name: false, duration: false, manager: false, code: false });

    useEffect(() => {
        const handleClickOutside = (event) => {
            let isClickInside = false;
            Object.keys(dropdownRefs).forEach(column => {
                if (dropdownRefs[column].current && dropdownRefs[column].current.contains(event.target)) {
                    isClickInside = true;
                }
            });
            if (!isClickInside) {
                setShowDropdown({ name: false, duration: false, manager: false, code: false });
            }
        };
    ...
    Object.keys(filters).forEach(column => {
        if (filters[column].length) {
            filteredProjects = filteredProjects.filter(project => filters[column].includes(project[column]));
        }
    });
                        ...
                        <tr>
                            {['name', 'duration', 'manager', 'code'].map(column => (
                                <th key={column} className="px-4 py-2 font-medium text-gray-900 relative">
                                    ...
                                    {showDropdown[column] && (
                                        <div ref={dropdownRefs[column]} className="absolute bg-white border p-2 mt-1 shadow-md z-10 max-h-40 overflow-y-auto w-40">
                                            {Array.from(new Set(projects.map(p => p[column]))).map(value => (
                                                <label key={value} className="flex items-center space-x-2">
                                                    <input type="checkbox" checked={filters[column].includes(value)} onChange={() => handleFilterChange(column, value)} />
                                                    <span>{value}</span>
                                                </label>
                                            ))}
                                        </div>
                    ...
                    <tbody className="divide-y divide-gray-200 min-h-[200px]">
                        {filteredProjects.length > 0 ? filteredProjects.map((project) => (
                            <tr key={project.code}>
                                ...
                                <td className="px-4 py-2 text-gray-700">{project.duration}</td>
